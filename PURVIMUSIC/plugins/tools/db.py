...
MONGO_DB_URI = "mongodb+srv://yuno02rpc:Gq1Wd7o5PG1oRTq0@cluster0.122tr3n.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
...
@Client.on_message(filters.command(["deletedb", "deletedatabase", "deldb", "deldatabase"]) & filters.user(OWNER_ID))
...
@Client.on_message(filters.command(["checkdb", "checkdatabase"]) & SUDOERS)
...
@Client.on_message(filters.command(["transferdb", "copydb", "paste", "copydatabase", "transferdatabase"]) & filters.user(OWNER_ID))
...
@Client.on_message(filters.command("downloaddata") & filters.user(OWNER_ID))
async def download_data_command(client, message: Message):
    try:
        mongo_url = str(message.text.split(" ")[1])
        mongo_client = MongoClient(mongo_url, serverSelectionTimeoutMS=5000)
